...
import { updateScheduled } from "../../../api/tradeApi";

interface EditScheduleModalProps {
  onClose: () => void;
  selected?: ScheduledTradeResponse;
}

const to24hours = (dateStr: string | undefined): string => {
  if (!dateStr) return "";
...
export default function EditScheduleModal({
  ...
  const [isPercent, setIsPercent] = useState(
    selected?.percentage ? true : false,
  );
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<ScheduledTradeResponse>({
    defaultValues: {
      ...selected,
      account_id: undefined,
      scheduled_time: to24hours(selected?.scheduled_time),
    },
  });

  useEffect(() => {
    if (selected) setValue("account_id", selected?.account_id);
  }, [accounts, setValue, selected]);

  const onSumbit: SubmitHandler<ScheduledTradeResponse> = async (data) => {
    if (isPercent) setValue("units", undefined);
    else setValue("percentage", undefined);
    const response = await updateScheduled(data);
    if (response.status === 200) {
      onClose();
    }
          ...
          <div>
            ...
            <input
              ...
              {...register("scheduled_time", {
                ...
                validate: (value) =>
                  new Date(value) > new Date() || "Time must be in the future",
              })}
            />
            <span className="ml-1 text-sm text-red-500">
